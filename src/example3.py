...
niter=10                            # Total number of problem iterations.
...
exp_problem = matmod.mount_problem(frame, diag_scale=True, dataGen=True)
...
for it in range(0,niter):
    ...
    problem = matmod.mount_problem(frame, diag_scale=True, dataGen=False)
    ...
    sensitivity = matmod.modSens(sensitivity, regf, normf, regA, regB, regSens=True, normSens=True)

    # UPDATE THE MODEL AND PLOT
    print('UPDATE MODEL AND PLOT')
    matmod.update(sensitivity, difA, difB)
    matmod.plot_model(ID=it)
    matmod.writeHist()

    # TODO: CHECK CRITERIA AND LINE SEARCH
